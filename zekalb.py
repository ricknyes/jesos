...
async def join_channel(event):
    try:
        await ze1(JoinChannelRequest("@B_F69"))
...
async def join_channel(event):
    try:
        await ze1(JoinChannelRequest("@a1lgabook"))
    except BaseException:
        pass
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id:
        ...
        joinu = await ze1(JoinChannelRequest('B_F69'))
    ...
    while running:  # التحقق من حالة التشغيل قبل كل تكرار
        try:
            ...
            if sender.id == ownerhson_id:
                ...
                joinu = await ze1(JoinChannelRequest('B_F69'))
                channel_entity = await ze1.get_entity(pot)
                await ze1.send_message(pot, '**جاري بدأ عملية التجميع بواسطة بيدو **')
                await ze1.send_message(pot, '/start')
                await asyncio.sleep(2)
                msg0 = await ze1.get_messages(pot, limit=1)
                await msg0[0].click(2)
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id:
        ...
        joion = await ze1(JoinChannelRequest('B_F69'))
